...
def commit_func_alt(u, p):

    # check username and password in heads and commit accordingly

    if os.listdir('.vcs/users/'):

        with open('.vcs/users/' + u + '.txt') as f:
            fl = f.readline().replace('\n', '')
            lines = f.readlines()
            tl = lines[1].replace('\n', '')
            fourthl = lines[-1].replace('\n', '')

            input_password_hash = base64.b64encode(hashlib.sha512(str.encode(p)).digest()).decode()
            input_uid = base64.b64encode(hashlib.sha1(str.encode(u)).digest()).decode()
            ...
            if comparission_result:
                ...
                temp_val = False
                create_obj_file(u, input_uid, fourthl, temp_val)
